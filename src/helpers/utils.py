...
def load_yaml_config(config):
    ...
    config_path = project_root / 'src' / 'config' / config
    with open(config_path, 'r') as f:
        data = yaml.load(f, Loader=yaml.SafeLoader)
    return data


def read_csv_panda(file, cols, index = None, encoding: str = "utf-8"):
    df = pd.read_csv(file, sep=";", usecols=cols, encoding = encoding)
    if index:
...
def clean_region_name(name):
    ...
    name = str(name).upper()
    # Rimuove il prefisso "REGIONE " se presente
    if name.startswith("REGIONE "):
        name = name.replace("REGIONE ", "", 1)
    # Rimuove eventuali spazi bianchi extra all'inizio o alla fine
    return name.strip()

def clean_csv_files(file, col, text, new_name):
    # Carica il file con l'encoding corretto
    df = pd.read_csv(f'resources/{file}', sep=';', encoding='latin-1')
    df[col] = df[col].str.replace(pat=rf'{text}', repl='', case=False, regex=True).str.upper().str.strip()
    df.to_csv(f'{new_name}.csv', sep=';', index=False, encoding='utf-8')

def normalize_regions(file, cols, new_name):
    # Carica il file con l'encoding corretto
    df = pd.read_csv(f'resources/{file}', sep=';', encoding='latin-1')
    for col in cols:        
...
def import_region_name(left_df, right_df, new_name):
    merged_df = pd.merge(
    left_df,
    right_df,
    left_on='PR', 
    right_on='Sigla automobilistica', 
    how='left'
)
    merged_df.loc[merged_df["Sigla automobilistica"] == "TN", "Denominazione Regione"] = "TRENTO"
    merged_df.loc[merged_df["Sigla automobilistica"] == "BZ", "Denominazione Regione"] = "BOLZANO"
    merged_df.loc["VALLE D'AOSTA" in merged_df["Denominazione Regione"], "Denominazione Regione"] = "VALLE D'AOSTA"
    merged_df['Denominazione Regione'] = merged_df['Denominazione Regione'].str.replace(pat=rf'-', repl=' ', case=False, regex=True).str.upper().str.strip()

    merged_df.to_csv(f'{new_name}.csv', sep=';', index=False, encoding='utf-8')
"""
