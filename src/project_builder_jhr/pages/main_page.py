...
from src.project_builder_jhr.models.tax import TaxResult
...
def _show_sankey(t: TaxResult):
    ...
    flussi_attivi = [
        ...
        if f[2] > 0.01 and f[0] is not None and f[1] is not None  # skip zero AND missing nodes
    ]
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        valueformat=".0f",
        valuesuffix="€",  # % on hover, real € in labels
        link=dict(
            source=source,
            target=target,
            value=value,
